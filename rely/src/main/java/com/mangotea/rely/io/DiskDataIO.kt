...
//    }
    ...
    fun <V : Serializable> pump(context: Context, key: String, value: V): Boolean {
        return pumpBy<V>(context, key) { o ->
            o == value
        }
    }

    inline fun <V : Serializable> pumpBy(
        context: Context,
        key: String,
        predicate: ((V) -> Boolean)
    ): Boolean {
        ...
        list.find { predicate(it) }?.let {
            if (list.remove(it))
                return@pumpBy io.put(context, key, list)
        }
        return false
    }

    //删除所有符合条件的数据
    inline fun <V : Serializable> pumps(
        context: Context,
        key: String,
        predicate: ((V) -> Boolean)
    ): Boolean {
