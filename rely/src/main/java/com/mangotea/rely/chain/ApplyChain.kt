...
import com.mangotea.rely.mainCoroutine
...
fun <E> knot(block: suspend Box.() -> E) = chain { Unit }.end { block() }
 ...
 * @return [Chain] 任务链
 */
fun <S> Knot<*, S>.end(course: Block<S, Unit>) = this.then(course).let { Chain(it) }
...
//        w("cancel of knot index :${it.currentStep}")
        ...
        val way1: String? = take("start")
        //方法2取值为空时，返回指定的默认值，返回类型为默认值类型
        val way2 = "start" take "default value"
        d("${way1 == way2}")
        it.destory()
    }.call(mainCoroutine)
}